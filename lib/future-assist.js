 ...
 *
 * @returns {Promise<{state: 'synced'|'failed'|'disabled', id: string|null, error: string|null}>}
 */
export async function syncRegistrationToFutureAssist(payload) {
  if (futureAssistDisabled()) {
    return { state: 'disabled', id: null, error: null };
  }
  ...
  try {
    ...
    return { state: 'synced', id: data.id ? String(data.id) : null, error: null };
  } catch (err) {
    return {
      state: 'failed',
      id: null,
