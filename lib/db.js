...
export function databaseUrl() {
  ...
  // Score rather than rely on environment ordering: the direct (unpooled and
  // no-SSL) URLs are the wrong choice for serverless functions, and picking one
  // by accident exhausts Postgres connections under load.
  const badness = (name) => {
    let score = 0;
    if (/NON_POOLING|UNPOOLED/i.test(name)) score += 2;
    if (/NO_SSL/i.test(name)) score += 1;
    return score;
  };
